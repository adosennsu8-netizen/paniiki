...
export default function HomePage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      setIsPremium(true); // ベータ期間中は全機能無料開放（2026年5月末まで）
      setEmail(user.email || "");
      setUid(user.uid);
      ...
      <div style={{ padding:"16px 16px 100px" }}>
        ...
        {!isPremium && (
          <div style={{ background:"#fef3cd", borderRadius:16, padding:16, border:"1.5px solid #c9963a", marginBottom:12 }}>
            <div style={{ fontSize:14, fontWeight:700, color:"#c9963a", marginBottom:6 }}>? プレミアムにアップグレード</div>
            <div style={{ fontSize:11, color:"#5a7a65", lineHeight:1.8, marginBottom:12 }}>月額500円で投稿・コメント・発作サポートが使えます。</div>
