...
export default function MapPage() {
  ...
  const handleJoinToggle = async () => {
    ...
    const updateData: Record<string, unknown> = {
      joining: newJoining, comment: myComment, updatedAt: serverTimestamp(),
    };
    if (newJoining && myLat !== null && myLng !== null) {
      updateData.lat = myLat; updateData.lng = myLng;
    }
    await setDoc(doc(db, "mapUsers", uid), updateData, { merge: true });
    await setDoc(doc(db, "users", uid), { mapJoining: newJoining }, { merge: true });
    if (newJoining && myLat !== null && myLng !== null) await loadNearbyUsers(uid, myLat, myLng);
  };
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      const updateData: Record<string, unknown> = {
        joining: true, comment: newComment, updatedAt: serverTimestamp(),
      };
      if (myLat !== null && myLng !== null) { updateData.lat = myLat; updateData.lng = myLng; }
      await setDoc(doc(db, "mapUsers", uid), updateData, { merge: true });
      await setDoc(doc(db, "users", uid), { mapJoining: true, mapComment: newComment }, { merge: true });
      setIsJoining(true);
      setComment("");
      if (myLat !== null && myLng !== null) await loadNearbyUsers(uid, myLat, myLng);
  ...
  <div key={u.id}
    ...
    {u.comment && (
      ...
      <div style={{ margin:"12px 16px 0", background:"#fff", borderRadius:16, padding:16, border:"1px solid #c8e6d0", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
        <div style={{ fontSize:14, fontWeight:700, color:"#2d4a38", marginBottom:4 }}>💬 MAPにコメントを残す</div>
